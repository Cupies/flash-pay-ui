...
.login-container {
  ...
  background-size: cover; // 确保背景图全屏覆盖
  position: relative;

  // 背景叠加层（增强表单可读性，不改变原背景视觉）
  &::before {
    content: '';
    position: absolute;
    top: 0;
    ...
    width: 100%;
    height: 100%;
    background-color: rgba(0, 0, 0, 0.3); // 半透明遮罩，提升表单与背景对比度
    z-index: 1;
  }
  ...
  .loginbox {
    ...
    .registry-form {
      ...
      // 表单底部链接
      .form-footer {
        margin-top: 51px; // 保留原布局间距
        text-align: left;

        .registry-link {
          color: $primary-color;
          font-size: 16px; // 保留原字体大小
          cursor: pointer;
          transition: $transition;
    ...
    .login-form {
      width: 100%;
      margin-top: 0; // 清除额外间距，保留原布局

      // 表单项样式（移除原边框，优化为统一容器）
      .form-item {
        margin-bottom: 0;
        border-bottom: 1px solid $border-color; // 保留原底部边框
        padding-bottom: 8px;
        margin-top: 15px; // 保留原垂直间距
        transition: $transition;
